...
const { precision, font, barStart, barEnd, rowHeight } = config()

const tooltipTop = ref('10px')
...
watch(
  ...
  async () => {
    ...
    const barId = bar?.value?.config.id || ''
    if (!barId) {
      return
    }
...
const dotColor = computed(() => bar?.value?.config.style?.background || DEFAULT_DOT_COLOR)
